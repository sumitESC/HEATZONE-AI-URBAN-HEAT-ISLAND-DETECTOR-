...
export function Header({ onToggleSidebar }: { onToggleSidebar: () => void }) {
  ...
  const refreshMutation = useRefreshWeatherData({
    mutation: {
      onSuccess: (data) => {
        // Invalidate all weather and heatzone related queries to ensure consistency
        queryClient.invalidateQueries({ queryKey: ['/api/datasets/overview'] });
        queryClient.invalidateQueries({ queryKey: ['/api/heatzone'] }); // Invalidate all children of /api/heatzone
      ...
      onError: (error: any) => {
        ...
        toast({
          title: "Sync Failed",
          description: "Could not synchronize weather data. Please check your connection and API key.",
          variant: "destructive",
        });
      }
    }
